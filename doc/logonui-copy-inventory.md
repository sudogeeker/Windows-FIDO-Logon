...
更新日期：2026-09-06。下表为本轮精简后的源码文案；编号沿用首次盘点，便于继续逐条讨论。

范围：本项目 Credential Provider 的登录、解锁界面。Windows 自带系统文案不在本仓库中。英文和简体中文共用 28 个 UiTextId，其中 Subtitle、SelectUserAndPassword 保留为空，不显示。
...
| L06 | UsbSecurityKeyLabel | USB 安全密钥 | USB security key |
...
| L12 | Fido2UsbSecurityKeyFallback | FIDO2 USB 安全密钥 | FIDO2 USB security key |
| M02 | SelectUserAndPasswordPrompt | 请选择用户并输入密码。 | Select a user and enter the password. |
...
| M08 | SelectKeyAndTouch | 请选择安全密钥，按需输入 PIN。 | Select a key. Enter its PIN if needed. |
...
- FID_SMALL_TEXT 在用户名/密码、安全密钥和修改密码三个阶段均显示；初始内容为 M02，L02 只描述该字段本身，并非持久副标题。
- Connect 过程中的提示会同时通过 IQueryContinueWithStatus::SetStatusMessage 和 FID_SMALL_TEXT 提交给 LogonUI；后续消息会替换前一条，重置时清空。
...
- 登录结果回调、断开/取消、切换用户或设备、取消选择和完整重置都会清除对应 PIN 状态，不提供跨次复用。
...
- X02：制造商和产品名均不可用时，LogonUI 使用本地化的 L12 兜底名称；两个值可用时保持其动态内容。
- 用户名及设备制造商/产品名为动态内容，不是固定文案。
...
验证：CredentialUiTests 覆盖错误分类、小字字段的运行时替换、本地化设备兜底和 PIN 清空通知；真实安全桌面与 USB 设备的显示时序按 doc/test-plan.md 手动验证。
